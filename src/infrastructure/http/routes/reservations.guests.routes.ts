...
// ⚠️ Fallback: enquanto o Prisma Client não for regenerado,
// usamos any para acessar reservationGuest sem erro de tipagem.
const guestModel = prisma.reservationGuest;
...
router.get('/:id/guests', async (req, res, next) => {
  try {
    ...
    const list = await guestModel.findMany({
...
router.post('/:id/guests', async (req, res, next) => {
  try {
    ...
    // unique(reservationId, email) -> 409 se já existir
    try {
      ...
      // Prisma P2002 (duplicado)
      if (e?.code === 'P2002') {
        return res.status(409).json({
          error: { code: 'DUPLICATE', message: 'Este e-mail já está na lista de convidados desta reserva.' },
        });
      }
      throw e;
    }
...
router.post('/:id/guests/bulk', async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(input) || input.length === 0) {
      return res.status(400).json({ error: { code: 'VALIDATION', message: 'Envie ao menos 1 convidado.' } });
    }
    if (input.length > 100) {
      return res.status(400).json({ error: { code: 'VALIDATION', message: 'Máximo de 100 convidados por requisição.' } });
    }
    ...
    if (normalized.length === 0) {
      return res.status(400).json({ error: { code: 'VALIDATION', message: 'Nenhum convidado válido após normalização.' } });
    }

    // createMany com skipDuplicates exige UNIQUE(reservationId,email) (já criamos no schema)
    const { count } = await guestModel.createMany({
